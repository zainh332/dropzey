...
options.forEach(option => {
    ...
    <script>
        /* VARIABLES */
        var dao;var daoUsers;var daoDelegatee = []; var is_Admin = false;
        ...
        const indexMain = async () => { 
           ...
           console.log((performance.now() - timeStart) / 1000)
        ...
        const removeAdmin = (user, event) => {
            ...
            if(walletAddress == dao.owner) {
                 if(isSafeToml(user)) {
                        ...
                        const id = talk("Checking address", "norm")
                          //save the address to the toml
                          modifyDao(dao.url, dao.code, 'address', user + "|@$$@|" + "empty", async (status) => {
                                if(status) {
                                    talk("Admin removed successfully", "good", id)
                                    //save back the results
                                    if(dao.toml.ACCOUNTS != undefined) {dao.toml.ACCOUNTS = []}
                                    if(dao.toml.WALLET_NAMES != undefined) {dao.toml.WALLET_NAMES = []}
                                    if(!dao.toml.ACCOUNTS.includes(user)) {dao.toml.ACCOUNTS[dao.toml.ACCOUNTS.indexOf(user)] = null}
                                    dao.toml.WALLET_NAMES[user] = null
                                    setUp()
                                    stopTalking(3, id)
                                }
                                else {
                     ...
                     const msg  = "Invalid characters(\") present in address.<br> Please remove it and try again";
                     stopTalking(4, talk(msg,'fail'))
                 }
            }
            else{
        ...
        const drawAdminUser = (params) => {
            return `<div
                            ...
                            <div onclick='removeAdmin("${params.user}", event)' class="d-flex align-items-start align-md-items-end justify-content-start justify-content-md-end gap-2 mt-2 w-100">
                                <button class="btn btn-danger text-white text "><small>Remove</small></button>
                            </div>
                        </div>`
        }
        const drawMember = (params) => {
            const sFlg = (!is_Admin || (walletAddress == params.member)) && (walletAddress == params.member)
            return ` <div id="" class="d-flex justify-content-between">
                                            ...
                                            <div class="member-ban-ctn" style='${(sFlg) ? 'display:none':"" }'>
                                                ...
                                                <div class="member-ban-modal d-none" id='member-ban_${params.member}'>
                                                    ...
                                                    style='${(!is_Admin || (walletAddress == params.member)) ? 'display:none':"" }'>
