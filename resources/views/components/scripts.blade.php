...
<script>
    ...
    const daoContractId = 'CC3YHLEMQR7Q7AN2F5K4KJGZFKOENYGFWSOTI4LO4JBRMCABRVGNV4DC'  
    ...
    const createProposal = async (params = {}) => {
        try{
            if(walletAddress != "") {
                ...
                let budget = StellarSdk.nativeToScVal(params.budget * 1)
    ...
    **/
    const banDaoMember = async (params = {}) => {
        try{
            const res = await isAdmin(params.url)
    ...
    **/
    ...
    // xyu
    setTimeout(async () => {
   //   console.log(await isAdmin("http://babies.testing.lumosdao.io/.well-known/stellar.toml", "GC3EF5XJY553DPO4N6HWBGXHYVEFL6SZJPODZBAXR5IB2DH3UWALXMJY"))
    }, 5000)//
    ...
    **/
    ...
    const isAdmin = async (dao_url, user=walletAddress) => {
        //structure url
        try {
            url = window.location.protocol + "//<?php echo $_SERVER['HTTP_HOST']; ?>/.well-known/asset.php?type=isadmin&url=" + encodeURIComponent(dao_url) + "&user=" + user
